...
class FC_Model extends CI_Model{
	...
	public function _vieworder($param = '', $catalogue = ''){
		$param['select'] = (isset($param['select'])) ? $param['select'] : '';
		$attribute['query'] = '';
		if($catalogue['rgt'] - $catalogue['lft'] > 1){
			$_list_child = $this->_get_where(array(
		...
		$data = $this->db->query('
			...
			FROM `catalogues_relationship` as `att`
			INNER JOIN `'.$param['modules'].'` as `pr`
			WHERE `pr`.`trash` = 0 AND `pr`.`publish` = 1 AND `att`.`modules` = \''.$param['modules'].'\' AND `att`.`modulesid` = `pr`.`id`'.$attribute['query'].'
		')->result_array();
		$this->db->flush_cache();
		return $data;
	}

		public function _viewajax($param = '', $catalogue = ''){
		$param['select'] = (isset($param['select'])) ? $param['select'] : '';
		$attribute['query'] = '';
		if($catalogue['rgt'] - $catalogue['lft'] > 1){
			$_list_child = $this->_get_where(array(
		...
		$data = $this->db->query('
			...
			FROM `catalogues_relationship` as `att`
			INNER JOIN `'.$param['modules'].'` as `pr`
			WHERE '.$param['where'].' AND `pr`.`trash` = 0 AND `pr`.`publish` = 1 AND `att`.`modules` = \''.$param['modules'].'\' AND `att`.`modulesid` = `pr`.`id`'.$attribute['query'].'
		')->result_array();
		$this->db->flush_cache();
		return $data;
	}
