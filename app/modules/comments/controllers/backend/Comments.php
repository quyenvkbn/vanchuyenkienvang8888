...
class Comments extends FC_Controller{
	...
	public function view($page = 1){
		...
		if($config['total_rows'] > 0){
			...
			$page = $page - 1;
			$data['listComment'] = $this->BackendComments_Model->view(($page * $config['per_page']), $config['per_page'],array('parentid'=>0));
			if(count($data['listComment']) && is_array($data['listComment']) && isset($data['listComment'])){
				foreach($data['listComment'] as $key=>$val){
					$data['listComment'][$key]['post'] = $this->BackendComments_Model->view(($page * $config['per_page']), $config['per_page'],array('parentid'=>$val['id']));
				}
			}
		}
