...
class FrontendArticlesCatalogues_Model extends FC_Model{
	...
	public function Search( $start = 0, $limit = 0, $lang = 'vietnamese'){
		...
		if(!empty($country)){
			$this->db->where('(thitruong LIKE \'%"'.$country.'"%\')');
		}
		if(!empty($training)){
			$this->db->where('(congviec LIKE \'%"'.$career.'"%\')');
		}
		if(!empty($training)){
			$this->db->where('(thunhap LIKE \'%"'.$gender.'"%\')');
		}
		if(!empty($training)){
			$this->db->where('(dotuoi LIKE \'%"'.$level.'"%\')');
		}
		if(!empty($training)){
			$this->db->where('(hannophoso LIKE \'%"'.$salary.'"%\')');
		}
		if(!empty($training)){
			$this->db->where('(thituyen LIKE \'%"'.$contract.'"%\')');
		}
