...
class Catalogues extends FC_Controller{
	...
	public function Delete($id = 0){
		...
		if($this->input->post('delete')){
			...
			$this->form_validation->set_rules('cataloguesid_showhome', 'Danh mục', 'trim|required|callback__Ishome');
			if ($this->form_validation->run($this)){
	...
	// Check id có phải là danh mục được chọn để hiển thị ra trang chủ nếu phải thì không được xóa
	public function _Ishome(){
		$id = $this->input->post('cataloguesid_showhome');
		$CheckCataloguesArticles =  $this->Backendsystems_Model->CheckCataloguesArticles(14);
		if(!$CheckCataloguesArticles){
			$this->form_validation->set_message('_Ishome', 'Danh mục hiện đang được sử dụng để hiện thị ngoài Homepage, vui lòng bỏ chọn danh mục hiện thị ngoài Homepage tại phần Hệ Thống');
			return FALSE;
		}
		return TRUE;
	}
