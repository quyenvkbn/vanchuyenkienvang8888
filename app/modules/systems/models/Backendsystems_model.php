...
class BackendSystems_Model extends CI_Model{
	...
	//check catalogues articles show home
	public function CheckCataloguesArticles($id = ''){
		$this->db->select('keyword,content');
		$this->db->from('systems');
		$this->db->where('content',$id);
		$this->db->group_start();
			$this->db->where('keyword','homepage_about');
			$this->db->or_where('keyword','homepage_service');
			$this->db->or_where('keyword','homepage_news');
			$this->db->or_where('keyword','homepage_representative');
		$this->db->group_end();
		$result = $this->db->count_all_results();
		$this->db->flush_cache();
