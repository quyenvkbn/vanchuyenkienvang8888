<section class="content-header">
	...
	<ol class="breadcrumb">
		...
		<li class="active"><a href="<?php echo site_url('products/backend/catalogues/view');?>">Danh mục sản phẩm</a></li>
	</ol>
</section>
	  ...
	  <div class="box">
		<div class="box-header">
			<h3 class="box-title pull-right">
				<div class="btn-group add-sort">
					...
					<a href="<?php echo site_url('products/backend/catalogues/create');?>" class="btn btn-add btn-flat"><i class="fa fa-plus"></i> Thêm danh mục</a>
				</div>
			</h3>
			<div class="box-tools pull-left">
				<form method="get" action="<?php echo site_url('products/backend/catalogues/view');?>">
		...
		<div class="box-body table-responsive no-padding">
			...
			<table class="table table-hover" id="diagnosis-list">
				...
				<?php  
					$href = rewrite_url($item['canonical'], $item['slug'], $item['id'], 'products_catalogues');
				?>
				<tr>
					...
					<?php 
						$count_id = catalogues_relationship($item['id'], 'products', array('Backendproducts','BackendproductsCatalogues'), 'products_catalogues', $this->fclang);
					?>
					...
					<td>
						<a href="<?php echo site_url('products/backend/catalogues/set/publish/'.$item['id'].'?redirect='.urlencode(current_url())); ?>" title="" class="status-publish">
					...
					<td>
						<a href="<?php echo site_url('products/backend/catalogues/set/ishome/'.$item['id'].'?redirect='.urlencode(current_url())); ?>" title="" class="status-publish">
					...
					<td>
						<a href="<?php echo site_url('products/backend/catalogues/set/highlight/'.$item['id'].'?redirect='.urlencode(current_url())); ?>" title="" class="status-publish">
					...
					<td>
						<a href="<?php echo site_url('products/backend/catalogues/set/isfooter/'.$item['id'].'?redirect='.urlencode(current_url())); ?>" title="" class="status-publish">
					...
					<td class="text-right">
						<div class="btn-group">
							<a href="<?php echo site_url('products/backend/catalogues/delete/'.$item['id']).'?redirect='.urlencode(current_url());?>" class="btn btn-default <?php echo ($item['rgt'] - $item['lft'] > 1)?'disabled':'';?>"><span class="fa fa-trash"></span></a>
							<a href="<?php echo site_url('products/backend/catalogues/update/'.$item['id']).'?redirect='.urlencode(current_url());?>" class="btn btn-default"><i class="fa fa-edit"></i></a>
							<a href="<?php echo site_url('products/backend/catalogues/read/'.$item['id']).'?redirect='.urlencode(current_url());?>" class="btn btn-default"><i class="fa fa-eye"></i></a>
						</div>
					 </td>
				</tr>
				<?php } ?>
...
$(document).ready(function(){
	$('#btnsort').click(function(){
		$.post('<?php echo site_url('products/ajax/catalogues/sort')?>', $('#fcFrom').serialize(), function(data){
			location.reload();
		})
		return false;
	})
